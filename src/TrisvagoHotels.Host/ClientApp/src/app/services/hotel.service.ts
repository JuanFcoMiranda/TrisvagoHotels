...
export class HotelService {
  ...
  markAsFavorite(hotelId: number, hotel: Hotel) : Observable<Hotel> {
    hotel.destacado = true;
    return this.http.put<Hotel>(this.myAppUrl + this.myApiUrl + hotelId, JSON.stringify(hotel), this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.errorHandler)
  ...
  unmarkAsFavorite(hotelId: number, hotel: Hotel) : Observable<Hotel> {
    hotel.destacado = false;
    return this.http.put<Hotel>(this.myAppUrl + this.myApiUrl + hotelId, JSON.stringify(hotel), this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.errorHandler)
